...
from agents.utils.util import tile_images
...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ob, share_obs, reward, done, info, avail_action = env.step(data)
            ...
            remote.send((ob, share_obs, reward, done, info, avail_action))
        elif cmd == 'reset':
            ob, share_obs, avail_action = env.reset()
            remote.send((ob, share_obs, avail_action))
        elif cmd == 'render':
            if data == "rgb_array":
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, share_obs, rews, dones, infos, avail_actions = zip(*results)
        try:
            aa = np.stack(obs)
            ...
            print(obs)
            input("p")
        # print(np.stack(share_obs))
        # print(np.stack(rews))
        # print(np.stack(dones))
        # print(infos)
        # print(np.stack(avail_actions))
    ...
    def reset(self):
        for remote in self.remotes:
            remote.send(('reset', None))
        obs, share_obs, avail_actions = zip(*[remote.recv() for remote in self.remotes])
        return np.stack(obs), np.stack(share_obs), np.stack(avail_actions)
...
class DummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        obs, share_obs, rewards, dones, infos, avail_actions = map(np.array, zip(*results))
    ...
    def reset(self):
        ...
        obs, share_obs, avail_actions = zip(*results)
        return np.array(obs), np.array(share_obs), np.array(avail_actions)
