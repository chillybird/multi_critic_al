...
class ConvertToMultiEnv(object):
    ...
    def share_observation_space(self):
        if not self._share_observ_space:
            self._share_observ_space = [self._env.observation_space for i in range(self.n_agents)]
        return self._share_observ_space

    @property
    def action_space(self):
        if not self._action_space:
            self._action_space = [
                gym.spaces.Box(-np.inf * np.ones(self.n_actions), np.inf * np.ones(self.n_actions), dtype=np.float32)
    ...
    def observation_space(self):
        if not self._observ_space:
            self._observ_space = [
                gym.spaces.Box(-np.ones(self.observ_len), np.ones(self.observ_len), dtype=np.float32)
                for _ in range(self.n_agents)]
        return self._observ_space

    def step(self, actions):
        # multi_env_actions = np.concatenate(actions)
        multi_env_actions = [actions[0][0], actions[1][0], actions[2][0], actions[3][0],
                             actions[0][1], actions[1][1], actions[2][1], actions[3][1]]
        share_obs, reward, done, info = self._env.step(multi_env_actions)
        obs = self._convert_observ(share_obs)
        share_obs = np.array([share_obs for _ in range(self.n_agents)])
        reward = [reward for i in range(self.n_agents)]
        # done = [done for i in range(self.n_agents)]
        info = [info for i in range(self.n_agents)]
        return obs, share_obs, reward, done, info, self.get_avail_actions()

    def reset(self):
        share_obs = self._env.reset()
        obs = self._convert_observ(share_obs)
        share_obs = np.array([share_obs for _ in range(self.n_agents)])
    ...
    def _convert_observ(self, observ):
        ...
        :return:
        """
        base_observ = observ[:4]

        # 各条腿对应的电机角度，分别是左前，左后，右前和右后
        observ_left_front = base_observ
        observ_left_back = base_observ
        observ_right_front = base_observ
        observ_right_back = base_observ
        ...
        if self.args['use_angle_in_observation']:
            if self.args['use_signal_in_observation']:
                motor_angles = observ[12:]
            else:
                motor_angles = observ[4:]

            if self.args['multi_task']:
                motor_angles = motor_angles.reshape(self.n_agents, 2)

            observ_left_front = np.concatenate([observ_left_front, motor_angles[0] if self.args['multi_task'] else motor_angles])
            observ_left_back = np.concatenate([observ_left_back, motor_angles[1] if self.args['multi_task'] else motor_angles])
            observ_right_front = np.concatenate([observ_right_front, motor_angles[2] if self.args['multi_task'] else motor_angles])
            observ_right_back = np.concatenate([observ_right_back, motor_angles[3] if self.args['multi_task'] else motor_angles])

        return np.array([observ_left_front, observ_left_back, observ_right_front, observ_right_back])
