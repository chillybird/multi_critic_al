...
import gym
import time
import algorithms.mch_ppo.core as core
from agents.utils.logx import EpochLogger
from agents.utils.mpi_pytorch import setup_pytorch_for_mpi, sync_params, mpi_avg_grads
from agents.utils.mpi_tools import mpi_fork, mpi_avg, proc_id, mpi_statistics_scalar, num_procs
...
class MCPPOBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, r_dim, gamma=0.99, lam=0.95):
        ...
        self.ret_buf = np.zeros(core.combined_shape(size, r_dim), dtype=np.float32)
        self.val_buf = np.zeros(core.combined_shape(size, r_dim), dtype=np.float32)
...
def mc_ppo(env_fn, ac_kwargs=dict(), seed=0, 
    ...
    buf = MCPPOBuffer(obs_dim, act_dim, local_steps_per_epoch, n_rewards, gamma, lam)
    ...
    o, ep_ret, ep_len, reward_breakdown = env.reset(), np.zeros(n_rewards), 0, np.zeros(n_rewards)

    # Main loop: collect experience in env and update/log each epoch
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            o_step = torch.as_tensor(o, dtype=torch.float32)
            a, v, logp = ac.step(o_step)
            ...
            if terminal or epoch_ended:
                ...
                if timeout or epoch_ended:
                    o_step = torch.as_tensor(o, dtype=torch.float32)
                    _, v, _ = ac.step(torch.as_tensor(o_step, dtype=torch.float32))
                else:
                    v = np.zeros(n_rewards)
                buf.finish_path(v)
                if terminal:
...
if __name__ == '__main__':
    ...
    mc_ppo(lambda : gym.make(args.env), actor_critic=core.MLPActorCritic,
