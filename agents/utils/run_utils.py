...
from agents.utils.logx import colorize
from agents.utils.mpi_tools import mpi_fork, msg
from agents.utils.serialization_utils import convert_json
import base64
...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None, 
    ...
    def thunk_plus():
        if 'retro_env' in kwargs:
            import retro
            import retro_contest
            from spinup.sonic_utils.launch_utils import make_singleLevel_sth_env

            env_name = kwargs['retro_env']
            assert env_name in ['sth1_greenHillAct1', 'sth1_springYardAct3'], env_name + " Unsupported retro env launcher"

            if env_name == 'sth1_greenHillAct1':
                kwargs['env_fn'] = lambda : make_singleLevel_sth_env('GreenHillZone.Act1')
            elif env_name == 'sth1_springYardAct3': 
                kwargs['env_fn'] = lambda : make_singleLevel_sth_env('SpringYardZone.Act3')
            del kwargs['retro_env']
            if 'retro_env' in kwargs:
